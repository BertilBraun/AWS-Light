...
async def get_overview(_: UserSpec = Depends(get_current_user)) -> dict[str, object]:
    ...
    reserved_cpu = sum(node.usage.cpu_used for node in nodes)
    actual_cpu = sum(node.actual_usage.cpu_used for node in nodes)
    total_cpu = sum(node.spec.cpu_capacity for node in nodes)
    reserved_memory = sum(node.usage.memory_used_mb for node in nodes)
    actual_memory = sum(node.actual_usage.memory_used_mb for node in nodes)
    total_memory = sum(node.spec.memory_capacity_mb for node in nodes)
    warnings = _build_warnings(services, node_count, total_cpu, reserved_cpu)
    ...
    return {
        ...
        "nodes": {
            "total": node_count,
            "cpu_used": reserved_cpu,
            "cpu_reserved": reserved_cpu,
            "cpu_actual": actual_cpu,
            "cpu_capacity": total_cpu,
            "memory_used_mb": reserved_memory,
            "memory_reserved_mb": reserved_memory,
            "memory_actual_mb": actual_memory,
...
def _build_warnings(
    ...
    for service in services:
        ...
        if actual < desired:
            warnings.append(f"{service.spec.name} has {actual}/{desired} running replicas")
    return warnings
