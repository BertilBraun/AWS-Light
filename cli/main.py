...
def _request(method: str, path: str, *, auth: bool = True, **kwargs: Any) -> httpx.Response:
    assert method in {"GET", "POST", "PUT", "DELETE"}, f"Unsupported method: {method}"
    assert path.startswith("/api/v1/"), "Path must start with /api/v1/"
    url = f"{_api_url()}{path}"
...
def get(path: str, *, auth: bool = True, **kwargs: Any) -> httpx.Response:
    return _request("GET", path, auth=auth, **kwargs)


def post(path: str, *, auth: bool = True, **kwargs: Any) -> httpx.Response:
    return _request("POST", path, auth=auth, **kwargs)


def put(path: str, *, auth: bool = True, **kwargs: Any) -> httpx.Response:
    return _request("PUT", path, auth=auth, **kwargs)


def delete(path: str, *, auth: bool = True, **kwargs: Any) -> httpx.Response:
    return _request("DELETE", path, auth=auth, **kwargs)
...
def destroy(targets: tuple[str, ...]) -> None:
    if not targets:
        console.print("[red]Usage:[/red] aws-light destroy <manifest.yaml|service-name>")
        console.print("       aws-light destroy service|secret|bucket <name>")
        sys.exit(1)
    ...
    if len(targets) == 1:
        ...
        path = Path(target)
        if path.exists():
            _destroy_manifest(path)
            return
        _destroy_resource("service", target)
        return

    if len(targets) == 2:
        kind, name = targets
        _destroy_resource(kind, name)
        return

    console.print("[red]destroy accepts either one target or <kind> <name>[/red]")
    sys.exit(1)


def _destroy_manifest(path: Path) -> None:
    yaml_text = path.read_text()
    response = post("/api/v1/manifests/destroy", json={"yaml_text": yaml_text})
    for result in response.json():
        console.print(f"[red]destroyed[/red] {result['kind']}/{result['name']}")


def _destroy_resource(kind: str, name: str) -> None:
    normalized = kind.lower()
    if normalized in {"service", "services"}:
        delete(f"/api/v1/services/{name}")
        console.print(f"[red]destroyed[/red] Service/{name}")
        return
    if normalized in {"secret", "secrets"}:
        delete(f"/api/v1/secrets/{name}")
        console.print(f"[red]destroyed[/red] Secret/{name}")
        return
    if normalized in {"bucket", "buckets"}:
        delete(f"/api/v1/storage/buckets/{name}")
        console.print(f"[red]destroyed[/red] Bucket/{name}")
        return
    console.print(f"[red]Unknown destroy kind:[/red] {kind}. Use service, secret, or bucket.")
    sys.exit(1)
...
def status(service_name: str | None) -> None:
    if service_name:
        response = get(f"/api/v1/services/{service_name}")
        service = response.json()
        _print_service(service)
    else:
        response = get("/api/v1/services")
...
def _print_service(service: dict) -> None:  # type: ignore[type-arg]
    ...
    for replica in service["replicas"]:
        console.print(
            f"    [{replica['status']}] {replica['replica_id'][:12]}  "
            f"port={replica.get('host_port', '-')}"
        )
...
def secret_get(name: str) -> None:
    response = get(f"/api/v1/secrets/{name}")
...
def storage_ls(bucket: str | None) -> None:
    if bucket:
        response = get(f"/api/v1/storage/buckets/{bucket}/objects")
        for obj in response.json():
            console.print(f"{obj['key']}  ({obj['size_bytes']} bytes)")
    else:
        response = get("/api/v1/storage/buckets")
...
def storage_cp(source: str, destination: str) -> None:
    if source.startswith("s3://"):
        ...
        response = get(f"/api/v1/storage/buckets/{bucket}/objects/{key}")
        ...
        put(
            f"/api/v1/storage/buckets/{bucket}/objects/{key}",
            content=data,
...
def storage_rm(uri: str) -> None:
    ...
    delete(f"/api/v1/storage/buckets/{bucket}/objects/{key}")
